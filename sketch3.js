new p5(p => {
    ...
    let loveScale = 1;
    let loveDirection = 1;
    let loveActive = false; // will pulsate after delay
  
    let loveX, loveY, loveW = 60, loveH = 60;
    ...
    p.draw = () => {
      ...
      if (bgImg3) {
        p.image(bgImg3, 0, 0, p.width, p.height);
      } else {
        p.background(255);
      }
      p.imageMode(p.CENTER);
    ...
    p.mousePressed = () => {
      // Check if user clicked on love4u
      if (
